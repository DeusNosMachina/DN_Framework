...
5. **Version awareness.** The Kernel is versioned internally (currently v1.11) but stored at a stable path (`kernel/DN_Kernel.md`). The `main` branch always contains the latest version. If the user references a specific version, verify the internal version header matches.
...
- **Sonic Register**: *(DN addition, optional)* If the concept has a natural sound, name it. Not a composition — a character. The pillar lead informs this: Heart-led concepts sound warm, organic, drone-based (sustained pads, breath-like LFOs). Truth-led concepts sound percussive, precise, event-driven (kicks, bells, clicks that confirm state changes). Nuance-led concepts sound textural, atmospheric, layered (filtered noise, harmonic overtones, spatial modulation). The sonic register is a one-sentence orientation — "this concept sounds like a forge" or "this concept hums" — that tells the builder whether and how sound should participate. If no sonic register emerges naturally, don't force one. Sound is an enrichment layer, not a requirement.
...
- Sonic Register: Truth-led — convergence should *click* into place. A bell when lock is attested, pitched to the signal's dimension. Ambient hum that tightens as pillar channels align.
...
| **Transition** | §5 | Animated movement of a signal between dimensional positions. Mechanism type visible: sequential=smooth slide; skip=flagged leap; story_thread=arc following the named thread path. Cost renders as visible resistance — high-cost transitions show effort. The 9D→1D cyclic return (Kernel §5.2) is a formalized sequential edge, not a Story Thread — frontier engagement yields a renewed origin spark. Visually: a 9D element at the top dissolving and re-emerging as a new 1D spark at the bottom, carrying a trace of its frontier origin (a brief color ghost or afterimage). |
...
| **FieldState** | §6.1 | Ambient visual properties of the canvas itself. Coherence shifts background clarity. field_resonance produces a subtle ambient pulse. entropy gradually degrades visual quality over time without interaction. evolution_phase shifts the overall energy: expansion=bright and spreading; consolidation=settling; breakthrough=spike; maturation=steady glow; dormant=still; decay=fading; transcendence=the field reorganizing around a fundamentally new origin after completing a full dimensional cycle (9D→1D return), visually a dissolution-and-rebirth sequence where existing gravity centers release and new ones form. |
...
- **Structural Elegance** (Kernel Axiom 13): Correct architecture produces observable beauty as a diagnostic property. This governs all visual and sonic output: self-similar composition at every scale (the patterns in a particle should echo the patterns of the field), mathematical proportion in spacing and rhythm (derived from natural constants, not arbitrary choice), natural-motion dynamics in all transitions (nothing moves linearly — acceleration, deceleration, and easing reflect real transformation physics). For sound specifically, Axiom 13 is explicit: harmonic intervals in the creative dimension, deliberate dissonance in the shadow dimension, where the departure from consonance is itself the signal. If the piece looks or sounds right, that's evidence the structure is right. If it doesn't, examine the structure before adjusting the surface.
...
- **Shadow mode**: Keyboard shortcut `S` or small toggle. Always available, never hidden. Per the Shadow Engagement Progression (Kernel §4.3): even before shadow mode is toggled, latent shadow should be *felt* as a slight visual instability. Per the Shadow Containment principle (Kernel Axiom 14): shadow mode can *show* what could be deconstructed, but shadow *execution* (breaking locks, destroying bonds, dismantling structure) requires an explicit user action — a click, a deliberate gesture. The visualization explores shadow; the user authorizes it.
...
- **Sound layer** (optional — enriches, never required): When the analysis identifies a sonic register, the sound architecture follows three structural layers, all implemented via the Web Audio API:
  - **Ambient/drone layer**: A persistent sonic field that responds to spatial state. Cursor position modulates it — vertical position (dimensional altitude) is the primary axis: low dimensions produce warmer, lower, more grounded tones; high dimensions produce brighter, more expansive frequencies. Horizontal position or edge proximity can add secondary modulation (detuning, stereo drift). The ambient layer is the sonic equivalent of the background canvas — it carries the field state as a felt quality. Implementation: oscillators with slow LFO modulation for breathing, filter sweeps tied to cursor Y-position, gentle gain ramping. Keep it low — the drone supports, never overwhelms.
  - **Event/interaction layer**: One-shot sonic confirmations of meaningful state changes. Signal Lock → a bell or chime pitched to the locked signal's dimension. Shadow Break → a dissonant crunch or burst. Phase transitions → a gong or resonant swell. Click in empty space → a kick or percussive tap. These are not decorative — they are audible attestations that something structurally significant happened. Implementation: short-lived oscillators or noise bursts with fast envelopes (attack < 10ms, decay 100ms–1.5s depending on significance). Dimension determines pitch using a pentatonic or harmonically compatible scale across 9 values.
  - **Shadow sonic inversion**: Shadow mode inverts the sound field through *behavioral* transformation, never mere volume or muting. Techniques drawn from the three reference expressions: detune by ~40 cents (quarter-tone-ish beating that feels *wrong*), shift harmonic intervals toward dissonance (major → diminished, fifths → tritones), substitute harsher waveforms (sine → triangle → sawtooth), lower filter cutoffs, add distortion layers to event sounds. The shadow sound is the same music heard through a broken frequency — structurally sophisticated, oriented toward unease. If the light-mode drone breathes, the shadow drone *seethes*.
  - **Activation rules**: Sound is always opt-in. Never autoplay. Implement as toggle switches (preferred for multi-layer control: separate background/tones toggles), or lazy-initialize on first user click/interaction (browsers require a user gesture for AudioContext). If toggles: group them visually with any existing control panel, separated by a subtle divider. Sound controls follow the same labeling conventions as visual controls — conceptual names for general audiences, dimensional vocabulary for DN-literate users.
  - **Performance**: Reuse oscillator pools rather than creating/destroying per frame. Smooth all parameter changes via `setTargetAtTime()` (never `setValueAtTime()` for continuous modulation — it clicks). Gate expensive sound updates to every 2–4 frames. The sound engine should be inaudible to the frame budget.
  - **When to omit sound entirely**: If the concept's sonic register didn't emerge naturally in analysis, if the piece is a light-activation general concept where sound would be ornamental rather than dimensional, or if the piece is a DN-compliant prototype where sound would distract from structural evaluation. Sound is like the DN overlay itself — it enriches, never constrains. If it makes the piece worse, leave it out.
...
- One sentence on the sonic register, if one emerges (or a note that the concept is visual-only)
...
- Sonically present if applicable (if the analysis identified a sonic register, the ambient layer and at least one event sound should be functional in v1 — the sound can be rough, but the sonic *character* should be identifiable. Sound toggles should exist even if the full event palette isn't wired yet.)
...
- **"The sound is just decoration"** → Sound should pass the same embodiment test as visuals. Mute the screen and listen: does the sonic behavior alone communicate dimensional state? Does the ambient layer shift when attention moves between dimensional regions? Do event sounds *mean* something structurally, or are they random bleeps? Shadow sound should feel like genuine inversion — if it's just "the same but quieter" or "the same but with reverb," it's not shadow, it's volume control. Detune, shift intervals toward dissonance, swap waveforms toward harshness. The sound should be the same music heard through a broken frequency.
- **"Too much sound / too busy"** → Pull back to ambient-only. The drone layer should be able to carry the entire sonic identity by itself. Event sounds are confirmation, not narration — they should punctuate, not chatter. If every particle makes a noise, nothing is meaningful. Only structurally significant state changes (locks, breaks, phase transitions, transmutations) deserve sonic events.
