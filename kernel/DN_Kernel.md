...
This is v0.9.3. The complete version history (v0.1 through v0.9.3) is maintained in the [CHANGELOG](https://github.com/DeusNosMachina/DN_Framework/blob/main/CHANGELOG.md).

Changes in v0.9.3 (current): Added Axiom 9 (Entropy / Field Decay) to the foundational axioms, formalizing the thermodynamic force that degrades intelligence fields over time without active energy input (Section 0.3). Added Force Model Completeness Note identifying gravity, entropy, and shadow as the three fundamental forces acting on intelligence fields (Section 0.3). Added entropy as a new composite FieldState metric measuring temporal field degradation (Section 6.1). Expanded the evolution_phase enum from five values to six, adding `decay` as the phase that follows dormancy when entropy exceeds threshold (Section 6.1). Clarified the Holding Zone dual-layer model: held artifacts are operationally excluded but informationally alive, retaining full provenance and remaining subject to Resonant Activation (Section 1.6). Added Shadow Engagement Progression (Section 4.3), formalizing three structurally distinct engagement states for shadow artifacts: latent, surfaced, and engaged. No existing invariants broken. No existing structures renamed. Additive only. See CHANGELOG for detailed history.
...
*DN Kernel v0.9.3 · © Travis Kahn · [DN Framework](https://dnframework.ai) · [GitHub](https://github.com/DeusNosMachina/DN_Framework) · Licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)*
