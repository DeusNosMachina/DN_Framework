...
This is v1.4. The complete version history (v0.1 through v1.4) is maintained in the [CHANGELOG](https://github.com/DeusNosMachina/DN_Framework/blob/main/CHANGELOG.md).

Changes in v1.4 (current): Refined entropy sub-signal model in Section 6.1 based on Worldlines Framework v7 convergence analysis. Reweighted entropy sub-signals to elevate unresolved_state_load (new, weight 0.30) as primary signal — measuring accumulation of unresolved computational debt. Demoted temporal_decay to second position (weight 0.25). Absorbed bridge_relevance_decay (v1.3, weight 0.20) into unresolved_state_load's fourth component (links whose rationale references materially changed conditions), restoring environmental_drift (weight 0.20) as a standalone sub-signal. Replaced snapshot_delta_velocity (v1.3, weight 0.15) with resolution_debt_ratio (weight 0.15) — measuring incomplete-to-total transition ratio — as a more independent measurement. signal_lock_aging (weight 0.10) unchanged. Updated evolution_phase decay detection thresholds to reference v1.4 sub-signals. Completed post-v1.2 vocabulary sweep correcting 19 surviving implementation-layer terms across Table 1, simulation commands, and prose descriptions. Added explicit human-presence requirement to Signal Lock Invariant (Section 1.5): sessions under AI-only facilitation may identify candidates but cannot commit lock events. Added Resonant Activation detection clarification to Holding Zone (Section 1.6): held signals may be included in simulation scope and Comparison analysis without influencing FieldState metrics. Added dual temporality clarification to evolution_velocity (Section 6.1): evolution metrics are computed from inter-Capture signal deltas, not from resolution_index arithmetic, and simulations should default to field_context as primary time frame. See CHANGELOG for detailed history.

Changes in v1.3: Self-audit of kernel v1.2 identified 17 findings across structural, inconsistency, and advisory severity tiers. Universalized all JSON schema keys in Section 8 from pre-v1.0 implementation terms to kernel terms (board_id → field_id, artifact_id → signal_id, bridge_type → link_type, snapshot_ids → capture_ids, zone_id → region_id, exercise_id → process_id, workspace_id → container_id, sections → domains, zones → regions, exercises → processes, artifacts → signals). Added missing FieldState metrics (entropy, dimensional_coherence, field_resonance, gravity_map, tension_map, evolution_breadth) to the Field Export example (Section 8.2). Fixed Axiom misattribution in Force Model Note (Section 0.3): Shadow corrected from Axiom 4 to Axiom 5. Fixed Table 1 Field required properties: sections[] → domains[]. Reconciled three-tier transition model (Section 5.3) to match Section 5 preamble: lower (1D→3D), middle (4D→5D), higher (6D→9D). Added completion_criteria status enum (met | partially_met | unmet | deferred | scope_changed | not_evaluated | open_exploration) to Section 1.6 structural requirement. See CHANGELOG for detailed history.
...
**AXIOM 6: Gravity Is Structural, Not Metaphorical.** The DN Code identifies "gravity" as the pull of intelligence toward denser meaning, the tendency of ideas, signals, and structures to cluster around high-resonance nodes. This is formalized in the Gravity Mapping simulation contract (Section 7.2) and the FieldState coherence metric (Section 6). Cognitive gravity is a measurable property of a field's intelligence field: which signals attract links, which regions accumulate density, which dimensions concentrate signal.
...
|---|---|---|
| **Signal** | Any discrete unit of intelligence captured in the system. The atomic element. | id, content (text), dimension { primary, shadow, nested }, position (nullable), pillar_lead, pillar_stabilizers[], created_at, process_id |
...
| **Saturation** | Density of signals relative to region capacity. Measures whether a region is underpopulated, balanced, or overcrowded. | Balanced signal distribution across regions. No empty regions alongside overflowing ones. | Critical regions empty. Peripheral regions bloated. Gravity concentrated in one area. |
...
The FieldState object is computed, not stored directly. It is generated on demand by analyzing the current state of signals, transitions, and links within a given scope (Field, Domain, or Region).

| **Field** | **Type & Description** |
|---|---|
| **scope_id** | ID of the Field, Domain, or Region being evaluated. |
...
| **saturation** | Float 0–1. Signal density relative to region capacity expectations. |
...
**Required fields:** id, source_id, target_id, link_type, rationale, dimension, scope. source_id and target_id may reference Signals, Processes, Regions, Domains, or Fields per Section 1.1 — Links may cross all containment boundaries. link_type is implementation-defined (e.g., resonance, tension, dependency, translation). rationale is required and must provide semantic justification for why the connection exists — Links without rationale are spec violations. scope is intra-field (default, both endpoints within the same Field) or cross-field (endpoints in different Fields within the same Container; see Section 8.12). position is nullable; when populated, it records the Link's spatial location within its host Field's topology (Section 1.1.2), typically computed as the midpoint between its source and target positions or explicitly placed by a participant.
...
**Required fields:** id, name, description, container_ids[], created_at, updated_at. The Environment is the outermost boundary in the containment hierarchy — the tenant-level isolation layer. container_ids[] references all Containers belonging to this Environment. tenant_isolation is a boolean indicating whether the Environment enforces strict data isolation from other Environments (default: true). metadata{} is an open object for implementation-specific tenant properties. The Environment does not carry FieldState, spatial_topology, or position — it is an administrative boundary, not an intelligence field. Cross-Environment interaction is not defined by this Kernel; Environments are structurally independent.
...
*DN Kernel v1.4 · © Travis Kahn · [DN Framework](https://dnframework.ai) · [GitHub](https://github.com/DeusNosMachina/DN_Framework) · Licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)*
