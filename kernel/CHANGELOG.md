...
- Fixed Axiom 6 (Section 0.3): "FieldState coherence metric" → "FieldState gravity_map metric." Gravity is formalized through `gravity_map`, not `coherence`; the two are structurally distinct metrics. Added modeling-stance clarification: gravity in DN refers to emergent attractor behavior produced by density and configuration within the field, modeled as a force for structural legibility.
